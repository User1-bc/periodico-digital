...
if (isset($_GET['eliminar_media'])) {
    ...
    $stmt_del = $conexion->prepare("SELECT archivo FROM noticias_multimedia WHERE id = ? AND noticia_id = ?");
    ...
    if ($archivo_info) {
        ...
        $stmt_drop = $conexion->prepare("DELETE FROM noticias_multimedia WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $update = $conexion->prepare("UPDATE noticias SET titulo = ?, descripcion = ? WHERE id = ?");
        ...
        for ($i = 0; $i < $total_archivos; $i++) {
            if ($_FILES['multimedia']['error'][$i] == 0) {
                ...
                if (move_uploaded_file($_FILES['multimedia']['tmp_name'][$i], $ruta_destino)) {
                    ...
                    $stmtMedia = $conexion->prepare("INSERT INTO noticias_multimedia (noticia_id, archivo, tipo) VALUES (?, ?, ?)");
