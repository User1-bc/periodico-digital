...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar_podcast'])) {
    ...
    if (!empty($video_id)) {
        ...
        try {
            $stmt = $conexion->prepare("INSERT INTO podcasts (titulo, descripcion, url_youtube) VALUES (?, ?, ?)");
...
$stmt_noticias = $conexion->query("SELECT * FROM noticias ORDER BY fecha_publicacion DESC");
$noticias = $stmt_noticias->fetchAll(PDO::FETCH_ASSOC);

// Obtener todos los anuncios
$stmt_anuncios = $conexion->query("SELECT * FROM anuncios ORDER BY id DESC");
$anuncios = $stmt_anuncios->fetchAll(PDO::FETCH_ASSOC);

// Obtener todos los podcasts
$stmt_podcasts = $conexion->query("SELECT * FROM podcasts ORDER BY id DESC");
